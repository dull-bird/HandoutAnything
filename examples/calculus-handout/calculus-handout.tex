...
% ── 出处与拓展阅读（紫色系，见 skill references/citations.md）──
\tcbuselibrary{skins}
\definecolor{refaccent}{HTML}{7c3aed}
\definecolor{refbg}{HTML}{f3e8fd}
\definecolor{readbg}{HTML}{faf5ff}
\newcommand{\src}[1]{%
  {\small\colorbox{refbg}{\color{refaccent}出处：#1}}}
\newtcolorbox{reading}{
  enhanced,
  colback=readbg, colframe=readbg, boxrule=0pt, arc=2pt,
  borderline west={2pt}{0pt}{refaccent},
...
\begin{itemize}
  \item 心理学·韦伯–费希纳定律：主观感受强度是物理刺激的对数函数 $S = k\ln(I/I_0)$（$I_0$ 是刚好能感觉到的最小刺激，叫绝对阈限）；\src{Fechner 1860}
  \item 心理学·史蒂文斯幂定律：$\psi = kI^{a}$，$a$ 因感觉通道而异（响度约 $0.67$，电击约 $3.5$）；\src{Stevens 1957}
...
韦伯定律说：能察觉的最小差别 $\Delta I$ 与当前刺激 $I$ 成比例，即 $\Delta I / I = $ 常数。提 1 kg 重物时多放 10 g 你能察觉；提 10 kg 时要多放 100 g 才察觉——同样的 $\Delta I$，刺激越大越“钝”。把这个比例关系累积起来（第 6 天会说“累积”就是积分），得到的正是对数函数：$S = k\ln(I/I_0)$。\textbf{对数不是数学家发明的记号，是“敏感度随基数递减”这条经验的必然形状。}Bernoulli 在 1738 年用同样的 $u(x)=\log x$ 描述财富的主观价值（边际效用递减）\src{Bernoulli 1738}——心理物理学和决策科学共享同一条曲线。
\end{keyconcept}
...
\textbf{本单元出处}
\begin{itemize}
  \item 函数与图像：《普林斯顿微积分读本》\footnote{下文简称为《读本》。全书版本信息（版次/译者/ISBN）见卷尾参考文献。}第 1 章；指数与对数函数的图像见第 9 章前半。
  \item 三种增长性格的学科例子（韦伯–费希纳、幂定律、复利）为讲义改编，原始出处已在正文行内标出。
\end{itemize}
\textbf{拓展阅读}
\begin{itemize}
  \item \textbf{想多练几道题}：OpenStax \emph{Calculus Vol.1} 第 1 章（免费 PDF，奇数题附答）——只读 1.2、1.5 两节即可，其余超纲。
  \item \textbf{想看动图理解指数与对数}：3Blue1Brown《微积分的本质》合集第 1 集，9 分钟，无需前置。
\end{itemize}
\end{reading}
...
这就是自然常数 $e$ 的定义（“连续复利”）。\src{《读本》第 9 章 §9.1}注意两件事：第一，$n$ 永远到不了“无穷大”，我们只是问\textbf{趋势}；第二，极限值 $e$ 不是任何一步算出来的结果，而是整条路线指向的目的地。记法 $\lim_{x\to a}f(x)=L$ 读作：$x$ 无限靠近 $a$ 时，$f(x)$ 无限靠近 $L$。
...
\subsection*{2.2 计算极限的三件事（考试 90\% 的题）}\src{《读本》第 3–4 章}
...
直觉定义：$f$ 在 $a$ 点连续 $\iff$ 极限值 $=$ 函数值，即 $\lim_{x\to a}f(x)=f(a)$。连续函数的图像一笔画成、没有跳断。为什么它有用？因为连续函数允许你“从中间值的存在性”推理：如果温度从 10 ℃ 连续升到 20 ℃，那必然经过 15 ℃（介值定理）\src{《读本》第 5 章 §5.1}——社会统计里“收入分布连续变化”这类论证背后都是它。
...
\textbf{本单元出处}
\begin{itemize}
  \item 极限直觉与计算三件事：《普林斯顿微积分读本》第 3–4 章；连续与介值定理见第 5 章 §5.1。
  \item 连续复利引出 $e$ 的路线与《读本》第 9 章 §9.1 的引入方式一致。
\end{itemize}
\textbf{拓展阅读}
\begin{itemize}
  \item \textbf{想看极限的严格定义}：《读本》第 3 章把直觉讲透后再去 OpenStax \emph{Calculus Vol.1} §2.2 看形式定义；学有余力才碰，卡住不影响后续。
  \item \textbf{想看动态逼近过程}：3Blue1Brown《微积分的本质》第 7 集（极限），10 分钟，先学完本讲再看。
\end{itemize}
\end{reading}
...
经济学把“多生产一单位，成本增加多少”叫边际成本——那是离散的说法。把产量看成连续变量，边际成本 $=$ 成本函数的\textbf{导数}；边际效用 $=$ 效用函数的导数。Bernoulli 的对数效用 $u(x)=\log x$ 的边际效用是 $u'(x)=1/x$：财富越多，每多一元带来的效用增量越小。你在经济学课背的“边际量递减/递增”，翻译成微积分就是“导数在变小/变大”。\footnote{《经济学原理》指曼昆《经济学原理（第 7 版）：微观经济学分册》（中译本）；版本信息见卷尾参考文献。}\src{《经济学原理》第 2 章}
\end{keyconcept}
...
\begin{itemize}
  ...
  \item 可导必连续，连续未必可导（$|x|$ 就是连续但尖点不可导的例子）。\src{《读本》第 5 章 §5.2}
\end{itemize}
\end{calcbox}
...
\textbf{本单元出处}
\begin{itemize}
  \item 导数定义（极限式）与用定义求导：《普林斯顿微积分读本》第 6 章 §6.1；可导与连续的关系见第 5 章 §5.2。
  \item “边际就是导数”的经济学表述参考曼昆《经济学原理》第 2 章，讲义做了微积分化的改写。
\end{itemize}
\textbf{拓展阅读}
\begin{itemize}
  \item \textbf{想看导数怎么被“看”出来}：3Blue1Brown《微积分的本质》第 1–2 集（导数与导数的悖论），各约 15 分钟，学完本讲再看。
  \item \textbf{想多练定义求导}：Paul's Online Math Notes，Calc I → Derivatives → Practice Problems，带逐步解答；前置就是今天的三步“展开→约分→代入”。
\end{itemize}
\end{reading}
...
$e^x$ 那行不是巧合：$e$ 就是被定义为“让指数函数的导数等于自身”的底数（这正是第 2 天连续复利那个 $e$ 如此特殊的深层原因）。组合规则：和差的导数 $=$ 逐项求导；常数倍直接提出来：$(3x^2+5e^x)'=6x+5e^x$。\src{《读本》第 6 章 §6.2；指数与对数求导见第 9 章 §9.3}
...
为什么合理？速度是“叠乘”的：$y$ 随 $u$ 变化的速度，乘上 $u$ 随 $x$ 变化的速度，就是 $y$ 随 $x$ 变化的速度——$\tfrac{dy}{dx}=\tfrac{dy}{du}\cdot\tfrac{du}{dx}$，链式法则正是这句话。\src{《读本》第 6 章 §6.2.5}

\begin{keyconcept}[链式法则实战：遗忘曲线降得有多快？]
现代常用的遗忘近似 $R(t)=e^{-t/S}$（$S$ 为记忆稳定性；此式出自 Woźniak 等 1995 年\src{Woźniak et al. 1995}，艾宾浩斯 1885 年本人用的是对数型公式，见书末资料注）。外层 $e^{u}$ 的导数是自身，内层 $u=-t/S$ 的导数是 $-1/S$，于是
\[
...
\textbf{本单元出处}
\begin{itemize}
  \item 和差、乘积、商与链式求导法则：《普林斯顿微积分读本》第 6 章 §6.2（链式法则在 §6.2.5）。
  \item $e^x$、$\ln x$ 的导数及 $a^x$ 的修正：《读本》第 9 章 §9.3。
\end{itemize}
\textbf{拓展阅读}
\begin{itemize}
  \item \textbf{想看链式法则为什么是“乘”}：3Blue1Brown《微积分的本质》第 4 集（链式法则），约 12 分钟，学完本讲再看。
  \item \textbf{想把求导算熟}：Paul's Online Math Notes，Calc I → Derivatives → Practice Problems，带逐步解答；前置就是今天的公式表加“外导乘内导”。
\end{itemize}
\end{reading}
...
\subsection*{5.1 导数的符号 = 函数的方向}\src{《读本》第 11 章}
...
\subsection*{5.2 求极值/最值三步}\src{《读本》第 11 章 §11.5；最值应用见第 13 章 §13.1}
...
最小二乘法：给数据点 $(x_i,y_i)$ 配直线 $y=mx+b$，总误差 $E(m,b)=\sum\bigl(y_i-mx_i-b\bigr)^2$。$E$ 是 $m$、$b$ 的开口向上的“碗”，碗底就是误差最小点；碗底的特征是每个方向的导数都为 $0$——对 $m$、$b$ 分别求导（多元时叫偏导，每次只动一个变量）并令其为 $0$，解出的正是回归系数公式。\textbf{“最小二乘”四个字里没有导数，但它的求解动作就是第 5.2 节那三步。}最大似然估计同理：似然函数（或对数似然 $\ell(\theta)$）取最大的参数 $\theta$，候选点满足 $\ell'(\theta)=0$。你以后在统计软件里点一下“回归”，背后跑的就是这一套。\src{最小二乘法自高斯、勒让德时代即为标准方法；“求导 $=0$”的求解动作见《读本》第 13 章 §13.1}
\end{keyconcept}
...
\textbf{本单元出处}
\begin{itemize}
  \item 导数符号与单调性、驻点分类：《普林斯顿微积分读本》第 11 章（§11.4–11.5）；最优化应用见第 13 章 §13.1。
  \item “回归即求导 $=0$”是讲义的综合改写；最小二乘法本身可查任何一本统计学教材的回归章节。
\end{itemize}
\textbf{拓展阅读}
\begin{itemize}
  \item \textbf{想看完整的画图流程}：《读本》第 12 章“如何绘制函数图像”，把今天的符号表方法用到最完整。
  \item \textbf{想多练最优化题}：Paul's Online Math Notes，Calc I → Applications of Derivatives → Optimization，带逐步解答。
\end{itemize}
\end{reading}
...
\subsection*{6.1 面积问题：无穷求和的入口}\src{《读本》第 15 章、第 16 章}
...
\subsection*{6.2 不定积分：求导的反操作}\src{《读本》第 17 章}
...
\subsection*{6.3 换元法：链式法则的逆用}\src{《读本》第 18 章 §18.1}
...
\textbf{本单元出处}
\begin{itemize}
  \item 定积分的“窄条求和取极限”定义：《普林斯顿微积分读本》第 15–16 章；原函数与 $+C$：第 17 章；换元积分：第 18 章 §18.1。
  \item 基尼系数与洛伦兹曲线面积的表述参考常见经济学入门教材，讲义做了积分化的改写。
\end{itemize}
\textbf{拓展阅读}
\begin{itemize}
  \item \textbf{想看积分怎么被“看”出来}：3Blue1Brown《微积分的本质》第 8 集（积分），约 11 分钟。
  \item \textbf{想把换元练熟}：Paul's Online Math Notes，Calc II → Integrals → Substitution，带逐步解答；前置是第 4 天的链式法则。
\end{itemize}
\end{reading}
...
\subsection*{7.1 全课最高光的定理}\src{《读本》第 17 章}
...
\subsection*{7.2 概率就是面积：通往统计的桥}\footnote{《概率论读本》指米勒《普林斯顿概率论读本》（中译本）；版本信息见卷尾参考文献。}\src{《概率论读本》第 8 章 §8.2}
...
\textbf{本单元出处}
\begin{itemize}
  \item 微积分基本定理（两个方向）：《普林斯顿微积分读本》第 17 章。
  \item “概率 $=$ 密度曲线下面积”与 68–95–99.7 经验法则：任何一本概率论教材的正态分布章节，如《概率论读本》第 8 章。
\end{itemize}
\textbf{拓展阅读}
\begin{itemize}
  \item \textbf{想直观看懂基本定理}：3Blue1Brown《微积分的本质》第 8–9 集，合计约 25 分钟。
  \item \textbf{想接着学积分技巧}：《读本》第 18 章（换元）与第 19 章（分部积分）；统计方向可接《心理统计》类课程。
\end{itemize}
\end{reading}
...
本节按专业引用规范（APA 格式改写）汇总本讲义用到的全部书目与文献：书名用中文（附原文名便于检索），论文保留原始英文标题。行内出处约定：论文写“作者姓 + 年份”（如 Stevens 1957），书写书名或简写 + 章节号（如《读本》第 9 章 §9.1），简写全称由首次出现处的脚注给出。各单元的行内出处与“本单元出处与拓展阅读”盒负责指到章节号，本节负责给全版本信息。

\textbf{书目}
\begin{itemize}[label={}, leftmargin=2em, itemindent=-2em, itemsep=3pt]
  \item Banner, A.（阿德里安·班纳）. (2016). \emph{普林斯顿微积分读本（修订版）}（杨爽、赵晓婷、高璞 译）. 人民邮电出版社. ISBN 978-7-115-43559-0.（英文原版 \emph{The Calculus Lifesaver: All the Tools You Need to Excel at Calculus}，Princeton University Press，2007）
  \item Mankiw, N. G.（曼昆）. (2015). \emph{经济学原理（第 7 版）：微观经济学分册}（梁小民、梁砾 译）. 北京大学出版社. ISBN 978-7-301-25690-9.（英文原版 \emph{Principles of Economics}，7th ed.，Cengage Learning，2014；本讲义对其“边际量”表述做了微积分化改写）
  \item Miller, S. J.（史蒂文·J.米勒）. (2020). \emph{普林斯顿概率论读本}（李馨 译）. 人民邮电出版社. ISBN 978-7-115-54377-6.
  \item Strang, G., \& Herman, E. (2016). \emph{Calculus Volume 1}（微积分·第一卷，开放教材）. OpenStax, Rice University. ISBN 978-1-938168-02-4.（CC BY-NC-SA 免费 PDF：openstax.org）
  \item Fechner, G. T.（费希纳）. (1860). \emph{心理物理学纲要}（\emph{Elemente der Psychophysik}）. Leipzig: Breitkopf \& Härtel.（19 世纪文献无 ISBN；第 1 天韦伯–费希纳定律的出处）
  \item Ebbinghaus, H.（艾宾浩斯）. (1885). \emph{记忆：实验心理学研究}（\emph{Über das Gedächtnis: Untersuchungen zur experimentellen Psychologie}）. Leipzig: Duncker \& Humblot.（19 世纪文献无 ISBN；遗忘曲线的原始出处，拟合公式见卷尾资料注）
\end{itemize}

\textbf{期刊论文与历史文献（保留原始标题，便于检索）}
\begin{itemize}[label={}, leftmargin=2em, itemindent=-2em, itemsep=3pt]
  \item Stevens, S. S. (1957). On the psychophysical law. \emph{Psychological Review}, 64(3), 153--181.（史蒂文斯幂定律的出处）
  \item Murre, J. M. J., \& Dros, J. (2015). Replication and analysis of Ebbinghaus' forgetting curve. \emph{PLOS ONE}, 10(7), e0120644.（艾宾浩斯遗忘曲线的当代复现与分析）
  \item Bernoulli, D. (1738). Specimen theoriae novae de mensura sortis. \emph{Commentarii Academiae Scientiarum Imperialis Petropolitanae}, 5, 175--192.（对数效用与边际效用递减的出处；英译：Exposition of a new theory on the measurement of risk. \emph{Econometrica}, 22(1), 23--36，1954）
  \item Woźniak, P. A., Gorzelańczyk, E. J., \& Murakowski, J. A. (1995). Two components of long-term memory. \emph{Acta Neurobiologiae Experimentalis}, 55(4), 301--305.（第 4 天遗忘曲线现代近似 $R(t)=e^{-t/S}$ 的出处）
\end{itemize}

其余配套资源（3Blue1Brown 视频、Paul's Online Math Notes、宋浩视频课等）不属正文引用，逐天映射见卷尾“资料注与后续学习指引”。
